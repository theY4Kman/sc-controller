...
class DS5Controller(HIDController):
    ...
    def __init__(self, device, daemon, handle, config_file, config, test_mode=False):
        ...
        self._output = DualSenseHIDOutput(
    ...
    def feedback(self, data):
        ...
        if position == HapticPos.LEFT:
            ...
            self._output.motor_left = half_amp
        elif position == HapticPos.RIGHT:
            self._output.motor_right = clamped_amp
        ...
        def clear_feedback(mapper):
            self._output.motor_right = self._output.motor_left = 0
            self.schedule_output()
    ...
    def flush(self):
        ...
        if self._output_dirty:
            data = bytes(bytearray(self._output).ljust(64, b'\x00'))
